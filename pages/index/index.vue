		...
		<view style="margin-bottom: 20rpx;">
			<swiper circular autoplay :interval="3000" :duration="500" indicator-dots style="height: 320rpx;"
				...
				<swiper-item v-for="item in imgs" :key="item">
					<image :src="item" style="width: 100%; height: 320rpx; border-radius: 8rpx;" mode="aspectFill" />
				</swiper-item>
			</swiper>
		</view>
...
<script>
	const defaultBanner = require('../../static/logo.png')
		...
		methods: {
			...
			loadBanners() {
				this.$request.get('/banner/selectAll').then(res => {
					...
					this.imgs = list.length ? list.map(item => item.url) : [defaultBanner]
				}).catch(() => {
					this.imgs = [defaultBanner]
				})
			},
			load() {
