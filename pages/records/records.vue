	...
	<view style="padding: 20rpx;">
		<view class="box" v-for="item in recordList" :key="item.id" style="margin-bottom: 10rpx;">
			<view style="display: flex; margin-bottom: 10rpx;">
				<view>
					<uni-tag type="success" text="领取" size="mini" v-if="item.type === '领取'"></uni-tag>
					<uni-tag type="primary" text="骑手" size="mini" v-if="item.type === '骑手'"></uni-tag>
					<uni-tag type="error" text="使用" size="mini" v-if="item.type === '支出'"></uni-tag>
					<uni-tag type="warning" text="退回" size="mini" v-if="item.type === '取消'"></uni-tag>
					<text style="margin-left: 10rpx;">{{ item.content }}</text>
				</view>
				<view style="flex: 1; text-align: right; font-weight: bold;">
					<text style="color: red;" v-if="item.type === '领取'">+{{ item.money }}</text>
